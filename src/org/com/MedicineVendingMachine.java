...
public class MedicineVendingMachine {
    public static void main(String[] args) throws SQLException, IOException {
        ...
        String que = "delete from medics where quantity = 0";
        PreparedStatement preparedStatement = con.prepareStatement(que);
        preparedStatement.executeUpdate();
        ResultSet check=stmt.executeQuery("select * from medics");
        if(!check.next()){
        	System.out.println("-------------------------------------------------------------------");
            System.out.println("+               Welcome to Medicine Vending machine               +");
            System.out.println("-------------------------------------------------------------------");
            System.out.println("\nSorry, Vending Machine is unavailable. No medicines present.");
        }
        else {
